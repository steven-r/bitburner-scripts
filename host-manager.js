import { log, getConfiguration, instanceCount, getNsDataThroughFile, scanAllServers, formatMoney } from './helpers.js'
...
export async function main(ns) {
    ...
    if (minRamExponent > maxPurchasableServerRamExponent) {
        log(ns, `WARN: --min-ram-exponent was set to ${minRamExponent} (${ns.formatRam(2 ** minRamExponent)}), ` +
            `but the maximum server RAM in this BN is ${maxPurchasableServerRamExponent} (${ns.formatRam(2 ** maxPurchasableServerRamExponent)}), ` +
        ...
        log(ns, `INFO: --min-ram-exponent is set to ${minRamExponent}: New servers will only be purchased ` +
            `if we can afford 2^${minRamExponent} (${ns.formatRam(2 ** minRamExponent)}) or more in size.`);
...
async function tryToBuyBestServerPossible(ns) {
    ...
    if (likelyHacknet.length > 0) {
        ...
        if (totalHacknetUsedRam == 0) {
            ...
            log(ns, `We are currently using ${ns.formatRam(totalHacknetUsedRam)} of hacknet RAM, so including hacknet in our utilization stats.`)
    }
    ...
    setStatus(ns, `Using ${ns.formatRam(totalUsedRam)}/${ns.formatRam(totalMaxRam)} (` +
    ...
    if (spendableMoney < cost)
        return setStatus(ns, `${prefix}spendableMoney (${formatMoney(spendableMoney)}) is less than the cost ` +
            `of even the cheapest server (${formatMoney(cost)} for ${ns.formatRam(2 ** exponentLevel)})`);
    ...
    if (exponentLevel < maxPurchasableServerRamExponent) {
        ...
        if (maxRamPossibleToBuy < ns.getServerMaxRam("home") * homeThreshold)
            return setStatus(ns, `${prefix}the most RAM we can buy (${ns.formatRam(maxRamPossibleToBuy)}) on our budget of ${formatMoney(spendableMoney)} ` +
                `is less than --compare-to-home-threshold (${homeThreshold}) x home RAM (${ns.formatRam(ns.getServerMaxRam("home"))})`);
        ...
        if (maxRamPossibleToBuy / totalMaxRam < networkThreshold)
            return setStatus(ns, `${prefix}the most RAM we can buy (${ns.formatRam(maxRamPossibleToBuy)}) on our budget of ${formatMoney(spendableMoney)} ` +
                `is less than --compare-to-network-ram-threshold (${networkThreshold}) x total network RAM (${ns.formatRam(totalMaxRam)})`);
    }
    ...
    if (worstServerName != null && maxRamPossibleToBuy < worstServerRam && !options['allow-worse-purchases'])
        return setStatus(ns, `${prefix}the most RAM we can buy (${ns.formatRam(maxRamPossibleToBuy)}) on our budget of ` +
            `${formatMoney(spendableMoney)} is less than our worst purchased server ${worstServerName}'s RAM ${ns.formatRam(worstServerRam)}`);
