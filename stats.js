...
function prepareHudElements(hudData) {
    ...
    for (const [header,, value, toolTip] of hudData) {
...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
    ...
    // augs/NF output
    {
        ...
        if (facManOutput && (facManOutput.affordable_augs.length || facManOutput.affordable_nf_count)) {
            ...
            facs.push(true, `${augs}/${facManOutput.affordable_nf_count}`, 'Affordable Augs/NFs');
        } else {
            facs.push(false);
        }
        hudData.push(facs);
    ...
    // Show various server / RAM utilization stats
    {
        ...
        if (!options['hide-RAM-utilization']) {
            ...
            val2.push(true, `${ns.formatRam(home.maxRam, 0)} ${ns.formatPercent(home.ramUsed / home.maxRam, 1)}`,
            ...
            val3.push(true, `${ns.formatRam(totalMax, 0)} ${ns.formatPercent(totalUsed / totalMax, 1)}`,
    ...
    {
        // show work progress
        const work = ['Work done'];
        const eta = ['- ETA'];
        const invite = ['Invite ETA'];
        ...
        if (currentWorkOutput !== '') {
            ...
            if (currentWork.work === 'faction' && currentWork.target_rep != 0) {
                work.push(true, `${ns.formatPercent(currentWork.current_rep / currentWork.target_rep, 1)}`, `Work done (${ns.formatNumber(currentWork.current_rep, 0)} / ${ns.formatNumber(currentWork.target_rep, 0)})`);
                // skip seconds
