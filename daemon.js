...
const argsSchema = [
    ...
    ['use-hacknet-servers', false], // Same as above
...
export async function main(ns) {
    ...
    useHacknetNodes = options.n || options['use-hacknet-servers'];
...
class Server {
    ...
    shouldHack() {
        return this.getMaxMoney() > 0 && this.name !== "home" && !this.name.startsWith('hacknet-server-') &&
...
export async function arbitraryExecution(ns, tool, threads, args, preferredServerName = null, useSmallestServerPossible = false, allowThreadSplitting = null) {
    ...
    // Push all "hacknet-server" servers to the end of the preferred list, since they will lose productivity if used
    var anyHacknetNodes = [];
    let hnNodeIndex;
    while (-1 !== (hnNodeIndex = preferredServerOrder.indexOf(s => s.name.startsWith('hacknet-server-'))))
