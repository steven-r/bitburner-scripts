 ...
 **/
export function formatMoney(num: number, maxSignificantFigures: number | undefined = 6, maxDecimalPlaces: number | undefined = 3) {
 ...
 * @param {number=} maxDecimalPlaces - (default: 3) The maximum decimal places you wish to see, regardless of significant figures. (e.g. 12.3, 1.2, 0.1 all have 1 decimal)
 **/
export function formatNumberShort(num: number, maxSignificantFigures: number | undefined = 6, maxDecimalPlaces: number | undefined = 3) {
    ...
    num = Math.abs(num);
    for (var i = 0; num >= 1000 && i < symbols.length; i++) num /= 1000;
 ...
 * @param {string} text - number to be parsed 
 **/
export function parseShortNumber(text: string = "0") {
 ...
 **/
export function formatNumber(num: number, minSignificantFigures: number | undefined = 3, minDecimalPlaces: number | undefined = 1) {
...
export function formatDateTime(datetime: Date) { return datetime.toISOString(); }
...
export function formatDuration(duration: number) {
    ...
    let sec = (duration / 1000.0)
    // Include millisecond precision if we're on the order of seconds
    const seconds = (hours == 0 && minutes == 0) ? sec.toPrecision(3) : sec.toFixed(0);
    if (sec > 0) {
...
export async function runCommand_Custom(ns: NS, fnRun: any, command: string, fileName: string, args: any[] = [], verbose = false, maxRetries = 5, retryDelayMs = 50) {
    ...
    return await autoRetry(ns, async () => {
        ...
        if (oldContents != script) {
            ...
            await autoRetry(ns, () => ns.read(fileName), (c: string) => c == script, () => `Temporary script ${fileName} is not available, ` +
 ...
 * @param {bool=} verbose - (default false) If set to true, pid and result of command are logged.
 **/
export async function waitForProcessToComplete(ns: NS, pid: number, verbose: boolean = false) {
 ...
 **/
export async function waitForProcessToComplete_Custom(ns: NS, fnIsAlive: any, pid: number, verbose: boolean = false) {
 ...
 * @param {NS} ns - The nestcript instance passed to your script's main entry point */
export async function autoRetry(ns: NS, 
 ...
 * @param {NS} ns - The nestcript instance passed to your script's main entry point */
export function log(ns: NS, message = "", alsoPrintToTerminal = false, toastStyle: string | undefined = undefined, maxToastLength = Number.MAX_SAFE_INTEGER) {
    checkNsInstance(ns, '"log"');
    ns.print(message);
    if (toastStyle) ns.toast(message.length <= maxToastLength ? message : message.substring(0, maxToastLength - 3) + "...", toastStyle as ToastVariant);
...
export async function tryGetBitNodeMultipliers_Custom(ns: NS, fnGetNsDataThroughFile: any) {
    ...
    try { return await fnGetNsDataThroughFile(ns, 'ns.getBitNodeMultipliers()', '/Temp/bitnode-multipliers.txt'); } catch { /* empty */ }
    return null;
}
...
export async function instanceCount(ns: NS, onHost = "home", warn = true, tailOtherInstances = true) {
    ...
    if (others.length >= 2) {
        ...
        if (tailOtherInstances) // Tail all but the last pid, since it will belong to the current instance (which will be shut down)
            others.slice(0, others.length - 1).forEach((pid: FilenameOrPID | undefined) => ns.tail(pid));
    }
 ...
 * @param {NS} ns */
export async function getStockSymbols(ns: NS) {
...
export async function getStocksValue(ns: NS) {
    ...
    const stockGetAll = async (fn: any) => await getNsDataThroughFile(ns,
    ...
    return stockSymbols.map((sym: string | number) => ({ sym, pos: positions[sym], ask: askPrices[sym], bid: bidPrices[sym] }))
        .reduce((total: number, stk: { pos: number[]; bid: number; ask: number; }) => total + (stk.pos[0] * stk.bid) /* Long Value */ + stk.pos[2] * (stk.pos[3] * 2 - stk.ask) /* Short Value */
...
export function getConfiguration(ns: NS, argsSchema: any[]) {
