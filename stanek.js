import {
    log, disableLogs, getFilePath, getConfiguration, formatNumberShort, 
    ...
    for (const fragment of fragmentsToCharge) {
        ...
        if (threads <= 0) {
            log(ns, `WARNING: Insufficient free RAM on ${currentServer} to charge Stanek ` +
                `(${ns.formatRam(availableRam)} free - ${ns.formatRam(reservedRam)} reserved). Will try again later...`);
            continue;
        }
        const pid = ns.run(chargeScript, threads, fragment.x, fragment.y);
        if (!pid) {
            log(ns, `WARNING: Failed to charge Stanek with ${threads} threads thinking there was ${ns.formatRam(availableRam)} free on ${currentServer}. ` +
