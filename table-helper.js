/* from https://github.com/xxxsinx/bitburner/blob/main/tables.js */
...
// helper for header/content align differenciation
const _HEADER = 'H';
const _CONTENT = 'C';

export const ALIGN_LEFT = 'left'; // align column left
export const ALIGN_RIGHT = 'right'; // align column right
export const ALIGN_CENTER = 'center'; // align column center

export const TABLE_SEPARATOR = '---'; // print a separator line
...
export async function main(ns) {
    ...
    data.push(TABLE_SEPARATOR); // print separator line
	...
	// pad   : and & right padding
    // align: align
	const columns = [
		{ header: 'Servers', width: 40, alignHeader: ALIGN_CENTER },
		{ header: 'Ram', width: 13, align: ALIGN_RIGHT, pad: 1 },
		{ header: 'Money', width: 20, pad: 1 }
	];
...
export function PrintTable(ns, data, columns, style = DefaultStyle(), printfunc = ns.print) {
	...
	if (columns == undefined) {
		...
		for (let i = 0; i < data[0].length; i++) {
			...
			columns.push({ header: '', width: longest, pad: 0, align: ALIGN_LEFT });
		}
	}
    fixupHeader(columns)
 ...
 * @param {Array} columns 
 */
function fixupHeader(columns) {
    for (var i = 0; i < columns.length; i++) {
        columns[i]._align = [];
        columns[i]._align[_HEADER] = columns[i].alignHeader || columns[i].align || ALIGN_LEFT;
        columns[i]._align[_CONTENT] = columns[i].alignContent || columns[i].align || ALIGN_LEFT;
        columns[i].pad = columns[i].pad || 0;
    }
}
...
function PrintBorder(ns, columns, style, printfunc = ns.print) {
	...
	for (let c = 0; c < columns.length; c++) {
		printStack.push('white', ''.padEnd(columns[c].width + columns[c].pad*2, style[FILLER]));
...
function PrintHeader(ns, columns, style, printfunc = ns.print) {
	...
	for (let c = 0; c < columns.length; c++) {
        const value = formatValue(columns[c], columns[c].header, _HEADER);
		printStack.push('white', value);
		printStack.push('white', style[BAR]);
	}
	PrintStack(ns, printStack, printfunc);
}
...
function PrintLine(ns, columns, data, style, printfunc = ns.print, highlight) {
	...
	for (let c = 0; c < columns.length; c++) {
		if (data[c].style != undefined)
			printStack.push({ style: data[c].style }, formatValue(columns[c], data[c].text, _CONTENT));
 ...
 * @returns {string}
 */
function formatValue(column, value, part) {
    if (value === undefined) {
        value = '';
    }
    ...
    let right;
    let spc;
    switch(column._align[part]) {
        case ALIGN_LEFT:
            left = column.pad;
            right = column.pad+column.width -len;
            break;
        case ALIGN_RIGHT:
            right = column.pad;
            left = column.pad+column.width -len;
            break;
        case ALIGN_CENTER:
            spc = Math.max((column.width -len) / 2, 0);
            left = column.pad + spc;
            right = column.pad+ spc;
            break;
        default:
            throw new Error("Invalid alignment" + column._align[part]);
    }
    return value.padStart(left+len).padEnd(right+left+len);
}
